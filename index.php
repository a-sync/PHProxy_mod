...
$pwd=md5('jelszó');//a jelszó
$hint='Jelszó emlékeztető';//elmékeztető
$btn='Mehet';//a gomb szövege
if($_COOKIE['p']!=$pwd)
{
  ...
  $foot = '</body></html>';
	if(!$_POST['s'])
	{
		die($head.'<form title="'.$hint.'" action="" method="post"><center>Jelszó:&nbsp;<input type="password" name="p"/>&nbsp;<input type="submit" name="s" value="'.$btn.'"/></center></form>'.$foot);
	}
	elseif(md5($_POST['p'])==$pwd)
	{
    $time = time() + 60 * 15;//15 percen át érvényes a süti
    setcookie('p', md5($_POST['p']), $time, '/', $_SERVER['HTTP_HOST']);
	}
	else
	{
		die($head.'<center><b>HIBA:</b> Helytelen jelszó.<br><a href="javascript:window.history.back()"><b>Vissza</b></a></center>'.$foot);
	}
...
$_config            = array
                    (
                        'url_var_name'             => 'l',
                        'flags_var_name'           => 'f',
                        'get_form_name'            => '_oh_noes_wtf_',
                        'basic_auth_var_name'      => '_trolololololo_',
                        'max_file_size'            => -1,
                        'allow_hotlinking'         => 0,
                        'upon_hotlink'             => 1,
                        'compress_output'          => 0
